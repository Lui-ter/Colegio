...
class cursoController extends Controller
{
    ...
    function formeditarC($id_curso){
        $curso = cursos::findOrFail($id_curso);
        $profesores = profesores::all();
        $estudiantes = estudiantes::all();
        return view ('interfaces/cursos/editar_curso', compact('curso','profesores','estudiantes'));
    }

    function editarC(Request $request, $id_curso){
        $validator = Validator::make($request->all(),[
            'nombre' => 'required|alpha:ascii|unique:cursos,nombre,'.$id_curso.',id_curso',
            'profesor_id' => 'required|exists:profesores,id_profesor',
            'estudiante_id' => 'required|exists:estudiantes,id_estudiante',
        ], [
            'nombre.required' => 'Se requiere un nombre al campo',
